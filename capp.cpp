...
bool cApp::OnInit()
{
    ...
    std::for_each(std::begin(tmp->m_item_map), std::end(tmp->m_item_map), [&](const auto& v) {
        tmp->m_items.push_back(v.second);
    });
    // sort the list by name, then by id
    std::sort(std::begin(tmp->m_items), std::end(tmp->m_items), [](const auto& first, const auto& second) {
        if (first->name != second->name)
            return first->name < second->name;
        return first->external_id < second->external_id;
    });
    // update the choice box with the names
    std::for_each(std::begin(tmp->m_items), std::end(tmp->m_items), [&](const auto& v) {
        all_element_names.emplace_back(v->name);
    });
